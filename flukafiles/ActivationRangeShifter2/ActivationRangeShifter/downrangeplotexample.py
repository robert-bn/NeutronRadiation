...
energies = np.linspace(0.07,0.250,19)  # GeV
...
for e in [0.10]:
    for t in [1,]:
        # set the directory
        dir = "/{rThickness}/{rThickness}-{bEnergy:03}.0/".format(rThickness=round(t), bEnergy=round(1000*e))
        ...
        if plotting:
            ...
            x = np.linspace(0.08,0.12,2000)
            yDown = ratio * gaussian(meanDownrange, std=np.sqrt(varDownrange))(x)
            yUp = gaussian(meanUprange, std=np.sqrt(varUprange))(x)

            plt.plot(x, yDown, c='k', ls='--', linewidth=1, label="Downstream energy distribution")
            plt.plot(x, yUp, c='k', linewidth=1, label="Upstream energy distribution")
